# Code by Andrew Ramsay: https://github.com/andrewramsay/ical_to_gcal_sync
...
def auth_with_calendar_api(config) -> Resource:
...
def auth_with_sheets_api(config) -> Resource:
    if config.get('LOGFILE', None):
        handler = logging.FileHandler(filename=config['LOGFILE'], mode='a')
    else:
        handler = logging.StreamHandler(sys.stderr)
